...
/**
 * @title eGold TRC20 — Gold-backed token for TRON
 * @notice 1 EGOLD = 1 gram of physical gold
 * @dev Compiled for TVM (TRON Virtual Machine)
 */
contract EGold_TRC20 {
    string  public name     = "eGold Token";
    ...
    // Gold price in INR with 6 decimal places
    // e.g. 5900000000 = ₹5,900.00 per gram
    ...
    constructor(uint256 _mintCap, uint256 _initialGoldPrice) {
        owner          = msg.sender;
        minter         = msg.sender;
    ...
    function updateGoldPrice(uint256 newPrice) external onlyOracle {
        require(newPrice > 0, "Price cannot be zero");
        uint256 old = goldPricePerGram;
        goldPricePerGram = newPrice;
        emit GoldPriceUpdated(old, newPrice);
    }
